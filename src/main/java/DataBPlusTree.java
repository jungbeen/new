...
public class DataBPlusTree<K, V> extends AbstractMap<K,V> implements SortedMap<K,V>
{
...
//	  @throws IllegalArgumentException thrown if order < 3 or leafOrder < 1.
	...
	DataBPlusTree(int order, int leafOrder) throws IllegalArgumentException
	{
	 ...
	 * @throws IllegalArgumentException thrown if order < 3 or leafOrder < 1.
	 */
	DataBPlusTree(Comparator c, int order, int leafOrder)
	...
	{
		if(comp instanceof DataBPlusTree.DefaultComparator)
			return null;
		else
			return comp;
	}
	...
	{
		Node cur = root;
		while(cur instanceof DataBPlusTree.GuideNode)
		{
...
//	 Associates the specified value with the specified key in this map. 
	...
	private class GuideNode extends Node
	{
		...
		{
			return DataBPlusTree.this.findGuideIndex(this, key);
		}
...
//			}
		...
		{
			return DataBPlusTree.this.findLeafIndex(this, key);
		}
	...
	private class SubMap extends AbstractMap<K,V> implements SortedMap<K,V>
	{
		...
		public boolean containsKey(Object key)
		{
			return checkKey(key) && DataBPlusTree.this.containsKey(key);
		}
		...
		{
			if(checkKey(key))
				return DataBPlusTree.this.get(key);
			else
				return null;
		}
		...
		{
			if(checkKey(key))
				return DataBPlusTree.this.put(key, value);
			else
		...
		{
			return DataBPlusTree.this.comparator();
		}
		...
		public SortedMap<K, V> subMap(K arg0, K arg1)
		{
			...
			// Return a new SubMap.
			return DataBPlusTree.this.subMap(newLow, newHigh);
		}
		...
		{			
			...
			{
				if(low == null && high == null)
					return DataBPlusTree.this.size();
				else
				{
					int count = 0;
			...
			private class EntrySetIterator implements Iterator<Entry<K,V>>
			{
				...
				{
					modCount = DataBPlusTree.this.modCount;
					curNode = DataBPlusTree.this.firstLeaf;
				...
				public Entry<K,V> next()
				{
					...
					if(modCount != DataBPlusTree.this.modCount)
					...
					// Get entry.
					lastEntry = new BPTEntry(curNode.keys.get(curIndex), DataBPlusTree.this);
	...
	private class BPTEntry implements Entry<K,V>
	{
	  ...
	  public static void main(String[] args) throws IOException {
		  ...
		  DataBPlusTree<Integer, String> dBt = new DataBPlusTree<Integer, String>();
		  ...
		  { 
			  ...
			  dataKey = st.nextToken();
			  dBt.put(Integer.parseInt(dataKey), readLine);
		  }
		  br1.close();
		  fr1.close();
